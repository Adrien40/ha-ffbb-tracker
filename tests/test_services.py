...
# ---------------------------------------------------------------------------
# Unknown entry_id must raise, not silently no-op
# ---------------------------------------------------------------------------


async def test_refresh_raises_on_unknown_entry_id(hass):
    """A typo'd or stale `entry_id` must surface an error, not do nothing."""
    _make_entry_with_coordinator(hass, "engagement-a")

    async_setup_services(hass)
    with pytest.raises(ServiceValidationError):
        await hass.services.async_call(
            DOMAIN, "refresh", {"entry_id": "does-not-exist"}, blocking=True
        )


async def test_get_next_matches_raises_on_unknown_entry_id(hass):
    """A typo'd or stale `entry_id` must surface an error, not an empty list."""
    _make_entry_with_coordinator(hass, "engagement-a")
...
async def test_get_standings_raises_on_unknown_entry_id(hass):
    """A typo'd or stale `entry_id` must surface an error, not an empty list."""
    _make_entry_with_coordinator(hass, "engagement-a")
